...
/// Apply a known workaround for a failed step.
pub async fn apply_workaround(
    host: &dyn RemoteHost,
    params: &RecoveryParams,
    ...
    match workaround {
        Workaround::ForceUninstallSoftNpu => {
            // Find and force-remove stuck SoftNPU zones
            ...
            for zone in &zone_list {
                if zone.name.contains("softnpu") || zone.name.contains("switch") {
                    let _ = tx
                        .send(RecoveryEvent::StepOutput(format!(
                            "Force-removing zone: {}",
                            zone.name
                    ...
                    let _ = host
                        .execute(&format!(
                            "pfexec zoneadm -z {} uninstall -F 2>/dev/null; pfexec zonecfg -z {} delete -F 2>/dev/null",
                            zone.name, zone.name
                        ))
                        .await;
                }
        ...
        Workaround::DestroyStaleZpools => {
            // Destroy any oxp_ zpools left from a previous install
            let output = host.execute("zpool list -Hp 2>/dev/null").await?;
            for line in output.stdout.lines() {
                let name = line.split('\t').next().unwrap_or("");
                if name.starts_with("oxp_") {
                    let _ = tx
                        .send(RecoveryEvent::StepOutput(format!(
                            "Destroying stale zpool: {name}"
                        )))
                        .await;
                    let _ = host
                        .execute(&format!("pfexec zpool destroy -f {name}"))
                        .await;
                }
            }
            Ok(())
        }
        Workaround::FixOwnership => {
            let cmd = format!(
                "pfexec chown -R $USER:staff ~/.cargo ~/.rustup {path}/target {path}/out 2>/dev/null",
                path = params.omicron_path
            );
            let _ = tx
                .send(RecoveryEvent::StepOutput("Fixing file ownership...".to_string()))
                .await;
            host.execute(&cmd).await?;
            Ok(())
        }
    }
}
